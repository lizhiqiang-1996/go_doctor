...
- **MR/PR 代码审查** — `--diff` 模式只扫描变更文件
- **Commit 代码审查** — `--commit` 模式扫描指定提交的变更
- **MCP Server** — 支持 Cursor / Claude Code / Windsurf 直接调用
...
Go Doctor 提供 MCP Server，支持 AI 编码工具直接调用。

```bash
go build -o go-doctor-mcp ./cmd/go-doctor-mcp/
```
...
```bash
claude mcp add go-doctor /path/to/go-doctor-mcp
```
...
| 工具 | 用途 |
|------|------|
| `scan_code_quality` | 完整扫描，支持 full/diff/commit 模式 |
| `get_code_score` | 快速获取评分 |
| `evaluate_ai_code_quality` | 评估 AI 生成代码的 5 维质量 |
  ...
  quality:
    ...
    steps:
      ...
      - uses: actions/setup-go@v5
        with:
          go-version: '1.21'
      - run: go install github.com/go-doctor/go-doctor/cmd/go-doctor@latest
